...
def affineRegistration(iType, rCP, iCP, iImage):
    ...
    if iType=='interpolation':
        ...
        if np.linalg.det(XY) !=0:
            ...
            raise ValueError('Matrix is not invertible, please select better points')
    ...
    if iCP.size:
        ...
        UV [0,:] = U
        UV [1,:] = V
        oCP = np.linalg.inv(oT) @ UV 
        oCP = oCP.T[:,:2]
    elif iType=='approximation':
        K = X.size
        x = X.sum()/K
        xx = (X*X).sum()/K
        ...
        XY = np.zeros((6,6))
        sub_matrix = np.array([[xx,xy,x],[xy,yy,y],[x,y,1]])
        XY[:3,:3] = sub_matrix
        XY[3:,3:] = sub_matrix
...
def getImages(iImage1, iImage2, iStep):
    ...
    oImage1 = np.abs(iImage1.astype(float) - iImage2.astype(float) +127).astype(np.uint8)
    # oImage1 = (iImage1.astype(float) - iImage2.astype(float)+255)/2

    oImage2 = np.array(iImage1)
    Y,X = iImage1.shape
    
    for y_idx, y in enumerate (range(0, Y, iStep)):
        for x_idx, x in enumerate (range(0, X, iStep)):
            isDiag = (y_idx + x_idx)%2
            if isDiag:
                y_lim = y+ iStep
                x_lim = x+ iStep
                if y_lim > Y:
                ...
                if x_lim > X:
                    x_lim = x
                oImage2[y : y_lim, x : x_lim] = iImage2[y : y_lim, x : x_lim]
...
def exhaustiveRegistration ( iImageA , iImageB , iTx , iTy ) :
    T = np.eye(3).astype(int)
    
    oTx = np.ones([(iTx[1]-iTx[0]/iTx[2]+1),1])
    oTy = np.ones([(iTy[1]-iTy[0]/iTy[2]+1),1])
    oMap = np.zeros([oTx.shape[0],oTy.shape[0]])

    Y,X = iImageA.shape
    counter_y = 0
    counter_x = 0
    for t_x in range(iTx[0],iTx[1] + iTx[2], iTx[2]):
         oTx[counter_x,1] = t_x
         
         for t_y in range(iTy[0],iTy[1] + iTy[1], iTy[2]):
            T[0,2]=t_x
            T[1,2]=t_y
            oTy[counter_y,1] = t_y
            MSE = 0
            tImage = transformImage(iType= 'affine', iImage = iImageB, iDim = [1,1], iP = T, iBgr =0, iInterp=1)
            for x in range(X):
                for y in range(Y):
            
                    if iImageB[y,x] > 0:
                        MSE = MSE  + (iImageA[y,x] - tImage[y,x])**2
                oMap[counter_x,counter_y] = MSE
            counter_y +=1
         counter_x +=1
